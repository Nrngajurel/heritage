...
@section('content')
    ...
    <div class="bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 px-4 py-16 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-7xl">
            @php
                $featuredMembers = \App\Models\TeamMember::featured()->get();
            @endphp
            ...
            <div class="space-y-24">
                @foreach($featuredMembers as $index => $member)
                    <div class="group relative overflow-hidden rounded-2xl p-8 transition-all duration-500">
                        <div class="relative z-10 flex items-center gap-12 {{ $index % 2 == 1 ? 'flex-row-reverse' : '' }}">
                            <div class="flex-1">
                                <h2 class="text-gold mb-4 text-4xl font-bold">{{ $member->name }}</h2>
                                <h3 class="text-gold/80 mb-6 text-2xl">{{ $member->position }}</h3>
                                ...
                                <div class="mb-8 flex space-x-4">
                                    ...
                                    @if($member->instagram_url && $member->instagram_url != '#')
                                        <a href="{{ $member->instagram_url }}" target="_blank" class="social-link">
                                            <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                                                <path
                            ...
                            <div class="flex-shrink-0">
                                <img src="{{ Storage::url($member->image) }}" alt="{{ $member->name }}" class="profile-image"
                                    loading="lazy">
                            </div>
                        </div>
                    </div>
            ...
            <!-- Team Members Grid -->
            @php
                $regularMembers = \App\Models\TeamMember::get();
            @endphp
            ...
            <div class="mt-24">
                ...
                <div class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
                    @foreach($regularMembers as $member)
                        <div
                            class="group relative overflow-hidden rounded-2xl bg-gray-800/20 backdrop-blur-sm transition-all duration-300">
                            <div class="relative overflow-hidden">
                                ...
                                <div
                                    class="absolute bottom-0 left-0 right-0 translate-y-4 p-6 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
                            ...
                            <div class="p-6">
                                <h3 class="text-gold mb-2 text-2xl font-bold">{{ $member->name }}</h3>
                                <p class="text-gold/60 text-lg">{{ $member->position }}</p>
                            </div>
                        </div>
                    @endforeach
